...
/**
     ...
     */    
private $parent_ids = []; // Array to store SKU => parent_id mapping
...
public function product_import_from_csv( $args, $assoc_args ) {
    ...
    while ( ( $data = fgetcsv( $handle, 0, ',' ) ) !== FALSE ) {
        ...
        // Check for attributes and sanitize
        $attributes = [];
        ...
        if ( $row['Type'] == 'variation' && ! empty( $this->parent_ids[ $row['SKU'] ] ) ) {
            $command .= ' --parent_id="' . esc_attr( $this->parent_ids[ $row['SKU'] ] ) . '"';
        }
        ...
        if ( ! empty( $row['Categories'] ) ) {
           ...
           // Implode categories back to string format
            ...
            // Convert any remaining HTML entities to characters
            $categories_string = htmlspecialchars_decode( $categories_string );
        ...
        // Check if it's a variable product and save parent_id
        if ( $row['Type'] == 'variable' ) {
            $result = WP_CLI::runcommand( $command, array( 'return' => true ) );
            if ( $result && is_numeric( $result ) ) {
                $this->parent_ids[ $row['SKU'] ] = $result; // Save parent ID for variable product
            }
        } else {
            WP_CLI::runcommand( $command, array( 'return' => true ) );
        }
